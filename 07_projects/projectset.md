...
///////// this is my task
    ...
    if (bmi <= 18.6){
      results.innerHTML = `you're underWeight ${bmi} please increase `;
      results.style.fontSize = "18px"
      results.style.fontWeight = "600"
    } else if (bmi > 18.6 || bmi <= 24.6){
      results.innerHTML = `you're Fit ${bmi} please keep it`;
      results.style.fontSize = "18px"
      results.style.fontWeight = "600"
    } else {
      results.innerHTML = `you're Fat ${bmi} please decrease`;
      results.style.fontSize = "18px"
      results.style.fontWeight = "600"
    }
  }
...
// js event control = setInterval
// format of setInterval === setInterval(function() {}, 1000)
...
let prevGuess = []   // user ke data ko store karke usi ko show karna
let numGuess = 1  // guess ko count karna

let playGame = true  // playGame for every game Coding
...
if(playGame){
  submit.addEventListener('click', function(e){
    ...
    console.log(guess);
    validateGuess(guess); // pass this to validateGuess 
  })
}

// take a guess no. from user for validating 
// user give a no. 1 to 100 rather than abc high of 100, smaller than 1 etc.
...
// check karenge ki correct hai ya nhi
// like (msg print on wrong answer, value low or high)

function checkGuess(guess){
  if(guess === randomNumber){
    displayMessage('you guesses it right') //call displayMessage
    endGame() // call endGame

  } else if (guess < randomNumber){
    displayMessage('Number is tooo LOW') //call displayMessage

  } else if (guess > randomNumber){ 
    displayMessage('Number is tooo HIGH') //call displayMessage
  }
...
// display of guess 
// over on guess cleanup game

function displayGuess(guess){
  userInput.value = ''
  guessSlot.innerHTML += `${guess}   `  // no. pushing
  numGuess++; // because start with 1
  remaining.innerHTML = `${11 - numGuess}`
}

// display here of checkGuess
// previous Guesses: increase
// Guesses Remaining: decrease
